...
var urlInput = {
  value: "https://smartrazorvideos.s3.us-west-1.amazonaws.com/1716357432058x7714422730230437001718602864401wPtFR"
}
...
function handleMessage({type, data}) { //dataUri, canvas, time}) {
    if (type === "startRender") {
      //startRender();
    }
    if (type === "init") {
...
//var myMp4boxfile = MP4Box.createFile();
...
function addBuffer(video, mp4track) {
	...
	if (MediaSource.isTypeSupported(mime)) {
      ...
      /*
			...
			sb.pendingAppends = [];
      */
      mp4boxfile.setSegmentOptions(track_id, null, { nbSamples: 1000 } );
...
function load() {
    ...
    console.log("WE GOT A SEGMENT MFERS")
	...
	downloader.setCallback(
		function (response, end, error) { 
			...
			if (response) {
				//progressbar.progressbar({ value: Math.ceil(100*downloader.chunkStart/downloader.totalLength) });
        //console.log(Math.ceil(100*downloader.chunkStart/downloader.totalLength));
	...
	downloader.setInterval(500);//parseInt(chunkTimeoutLabel.value));
	downloader.setChunkSize(1000000);//parseInt(chunkSizeLabel.value));
